  ...
  async getGeneralYearlyCollectionKPI(
    ...
    try {
      ...
      const query = `
        ...
        WHERE (@code = '' OR @code IS NULL OR di.Cod_Titulo_Datos = @code) AND di.${queryDateFilter} >= @startDate AND di.${queryDateFilter} <= @endDate
        GROUP BY
      ...
      const query = `
        ...
        FROM (
            SELECT  
                ...
                COUNT(Cod_Ingreso)                         AS record_count  
            FROM Datos_ingreso WITH (NOLOCK)
            WHERE ${queryDateFilter} >= CONVERT(DATETIME, '${startDateStr}', 120)
              AND ${queryDateFilter} <= CONVERT(DATETIME, '${endDateStr}', 120)
              ${titleCodeFilter}  
            GROUP BY  
