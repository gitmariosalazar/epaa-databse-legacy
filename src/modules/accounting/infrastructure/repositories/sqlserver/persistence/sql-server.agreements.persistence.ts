  ...
  implements InterfaceAgreementsRepository
{
  ...
  async getAgreementsKpi(
    ...
    try {
      ...
      const query = `
        SET NOCOUNT ON;
        DECLARE @SearchType VARCHAR(10) = '${searchType}';
        DECLARE @startDate DATETIME = CONVERT(DATETIME, '${startYear}-01-01 00:00:00.000', 120);
        DECLARE @endDate DATETIME = CONVERT(DATETIME, '${endYear}-12-31 23:59:59.997', 120);
        ...
        SELECT
            ...
            CAST(SUM(CASE WHEN estado_pago = 1 THEN valor_capital ELSE 0 END) * 100.0 / NULLIF(SUM(valor_capital), 0) AS DECIMAL(10,2)) AS principal_recovery_pct,
            CAST(SUM(CASE WHEN estado_pago = 1 THEN 1.0 ELSE 0.0 END) * 100.0 / NULLIF(COUNT(*), 0) AS DECIMAL(10,2)) AS collection_efficiency_pct,
            CAST(SUM(CASE WHEN estado_pago = 1 THEN (valor_capital + valor_interes + valor_recargo) ELSE 0 END) * 100.0 / NULLIF(SUM(valor_capital + valor_interes + valor_recargo), 0) AS DECIMAL(10,2)) AS collection_amount_pct,
            ...
            AVG(CASE WHEN estado_pago = 0 AND fecha_vencimiento < GETDATE() THEN DATEDIFF(day, fecha_vencimiento, GETDATE()) END) AS avg_overdue_days,
            MAX(CASE WHEN estado_pago = 0 AND fecha_vencimiento < GETDATE() THEN DATEDIFF(day, fecha_vencimiento, GETDATE()) END) AS max_overdue_days,
            SUM(CASE WHEN estado_pago = 0 AND fecha_vencimiento < GETDATE() AND DATEDIFF(day, fecha_vencimiento, GETDATE()) BETWEEN 1 AND 30 THEN 1 ELSE 0 END) AS overdue_1_30_days,
            SUM(CASE WHEN estado_pago = 0 AND fecha_vencimiento < GETDATE() AND DATEDIFF(day, fecha_vencimiento, GETDATE()) BETWEEN 31 AND 60 THEN 1 ELSE 0 END) AS overdue_31_60_days,
            SUM(CASE WHEN estado_pago = 0 AND fecha_vencimiento < GETDATE() AND DATEDIFF(day, fecha_vencimiento, GETDATE()) BETWEEN 61 AND 90 THEN 1 ELSE 0 END) AS overdue_61_90_days,
            SUM(CASE WHEN estado_pago = 0 AND fecha_vencimiento < GETDATE() AND DATEDIFF(day, fecha_vencimiento, GETDATE()) > 90 THEN 1 ELSE 0 END) AS overdue_more_90_days,
        ...
        GROUP BY YEAR(fecha_emision), CASE WHEN @SearchType IN ('MONTH','DAY') THEN MONTH(fecha_emision) ELSE NULL END, CASE WHEN @SearchType = 'DAY' THEN DAY(fecha_emision) ELSE NULL END
        ORDER BY year DESC, month DESC, day DESC;
      `;

      const rawData: AgreementKPIsSqlResult[] =
        await this.sqlServerService.query<AgreementKPIsSqlResult>(query);
      return new SqlServerAgreementsAdapter().adaptAgreementsKpiData(rawData);
    } catch (error) {
  ...
  async getAgreementsKpiCustomer(
    ...
    try {
      ...
      const query = `
SET NOCOUNT ON;
DECLARE @Card_Id VARCHAR(15) = '${cardId}';
DECLARE @SearchType VARCHAR(10) = '${searchType}';
DECLARE @startDate DATETIME = CONVERT(DATETIME, '${startYear}-01-01 00:00:00.000', 120);
DECLARE @endDate DATETIME = CONVERT(DATETIME, '${endYear}-12-31 23:59:59.997', 120);

SELECT
    YEAR(di.fecha_emision) AS year,
    CASE WHEN @SearchType IN ('MONTH','DAY') THEN MONTH(di.fecha_emision) ELSE NULL END AS month,
    CASE WHEN @SearchType = 'DAY' THEN DAY(di.fecha_emision) ELSE NULL END AS day,
    ...
    SUM(di.valor_capital + di.valor_interes + di.valor_recargo) AS total_emitted,
    SUM(CASE WHEN di.estado_pago = 1 THEN (di.valor_capital + di.valor_interes + di.valor_recargo) ELSE 0 END) AS total_collected,
    SUM(CASE WHEN di.estado_pago = 0 THEN (di.valor_capital + di.valor_interes + di.valor_recargo) ELSE 0 END) AS total_pending,
    SUM(di.valor_capital)      AS total_principal,
    SUM(di.valor_interes)      AS total_interest,
    SUM(di.valor_recargo)      AS total_surcharge,
    SUM(CASE WHEN di.estado_pago = 1 THEN di.valor_capital ELSE 0 END) AS principal_collected,
    CAST(SUM(CASE WHEN di.estado_pago = 1 THEN di.valor_capital ELSE 0 END) * 100.0 / NULLIF(SUM(di.valor_capital), 0) AS DECIMAL(10,2)) AS principal_recovery_pct,
    CAST(SUM(CASE WHEN di.estado_pago = 1 THEN 1.0 ELSE 0.0 END) * 100.0 / NULLIF(COUNT(*), 0) AS DECIMAL(10,2)) AS collection_efficiency_pct,
    CAST(SUM(CASE WHEN di.estado_pago = 1 THEN (di.valor_capital + di.valor_interes + di.valor_recargo) ELSE 0 END) * 100.0 / NULLIF(SUM(di.valor_capital + di.valor_interes + di.valor_recargo), 0) AS DECIMAL(10,2)) AS collection_amount_pct,
    COUNT(DISTINCT di.ciudadano_id)        AS total_citizens_with_agreements,
    COUNT(*)                            AS total_installments_count,
    SUM(CASE WHEN di.estado_pago = 0 THEN 1 ELSE 0 END ) AS total_installments_pendings,
    SUM(CASE WHEN di.estado_pago = 1 THEN 1 ELSE 0 END ) AS total_installments_paid,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() THEN 1 ELSE 0 END) AS overdue_installments_count,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() THEN (di.valor_capital + di.valor_interes + di.valor_recargo) ELSE 0 END) AS overdue_amount,
    AVG(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() THEN DATEDIFF(day, di.fecha_vencimiento, GETDATE()) END) AS avg_overdue_days,
    MAX(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() THEN DATEDIFF(day, di.fecha_vencimiento, GETDATE()) END) AS max_overdue_days,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() AND DATEDIFF(day, di.fecha_vencimiento, GETDATE()) BETWEEN 1 AND 30 THEN 1 ELSE 0 END) AS overdue_1_30_days,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() AND DATEDIFF(day, di.fecha_vencimiento, GETDATE()) BETWEEN 31 AND 60 THEN 1 ELSE 0 END) AS overdue_31_60_days,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() AND DATEDIFF(day, di.fecha_vencimiento, GETDATE()) BETWEEN 61 AND 90 THEN 1 ELSE 0 END) AS overdue_61_90_days,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() AND DATEDIFF(day, di.fecha_vencimiento, GETDATE()) > 90 THEN 1 ELSE 0 END) AS overdue_more_90_days,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < DATEADD(month, -3, GETDATE()) THEN 1 ELSE 0 END) AS critical_overdue_count,
    SUM(CASE WHEN di.estado_pago = 0 THEN di.valor_capital ELSE 0 END) AS capital_balance_pending,
    AVG(di.valor_capital + di.valor_interes + di.valor_recargo) AS avg_installment_value,
    AVG(CASE WHEN di.estado_pago = 1 THEN DATEDIFF(day, di.fecha_emision, di.fecha_pago) END) AS avg_days_to_pay,
    MIN(di.fecha_emision) AS first_installment_date,
    MAX(di.fecha_emision) AS last_installment_date,
    MIN(di.fecha_vencimiento) AS oldest_due_date,
    COUNT(DISTINCT di.id) AS total_agreements,
    SUM(CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento >= GETDATE() THEN 1 ELSE 0 END) AS pending_not_overdue
FROM Datos_Ingreso_Convenio di
INNER JOIN CIUDADANO c ON di.ciudadano_id = c.CED_IDENT_CIUDADANO
WHERE di.fecha_emision BETWEEN @startDate AND @endDate AND di.ciudadano_id = @Card_Id
GROUP BY di.ciudadano_id, c.NOMBRES_CIUDADANO, c.APELLIDOS_CIUDADANO, c.EMAIL_CIUDADANO, c.TELEFONO_CIUDADANO, YEAR(di.fecha_emision), CASE WHEN @SearchType IN ('MONTH','DAY') THEN MONTH(di.fecha_emision) ELSE NULL END, CASE WHEN @SearchType = 'DAY' THEN DAY(di.fecha_emision) ELSE NULL END
ORDER BY year DESC, month DESC, day DESC;
      `;
      ...
      console.error('Error fetching agreements KPI for customer:', error);
  ...
  async getAgreementInstallmentDetails(
    ...
    try {
      ...
      const query = `
          SET NOCOUNT ON;
          DECLARE @Card_Id VARCHAR(15) = '${cardId}';
          DECLARE @startDate DATETIME = CONVERT(DATETIME, '${initDateTime}', 120);
          DECLARE @endDate DATETIME = CONVERT(DATETIME, '${endDateTime}', 120);
          ...
          SELECT
              ...
              CASE WHEN di.estado_pago = 1 THEN di.fecha_pago ELSE NULL END AS payment_date,
              ...
              CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < GETDATE() THEN DATEDIFF(DAY, di.fecha_vencimiento, GETDATE()) ELSE 0 END AS days_overdue,
              CASE
                  WHEN di.estado_pago = 1 THEN 'PAID'
              ...
              CASE WHEN di.estado_pago = 0 AND di.fecha_vencimiento < DATEADD(MONTH, -3, GETDATE()) THEN 'CRITICAL' ELSE 'NORMAL' END AS risk_level,
              di.valor_capital AS pending_principal,
              DATEDIFF(DAY, di.fecha_emision, GETDATE()) AS days_since_issue,
              CASE WHEN di.estado_pago = 1 THEN DATEDIFF(DAY, di.fecha_emision, di.fecha_pago) ELSE NULL END AS days_to_payment
          FROM Datos_Ingreso_Convenio di
          INNER JOIN CIUDADANO c ON c.CED_IDENT_CIUDADANO = di.ciudadano_id
          WHERE ciudadano_id = @Card_Id AND fecha_emision BETWEEN @startDate AND @endDate
      ...
      console.error('Error fetching agreement installment details:', error);
  ...
  async getMonthlyCollectionSummary(
    ...
    try {
      const query = `
        SET NOCOUNT ON;
        DECLARE @MonthsBack INT = ${monthsBack};
        SELECT
            FORMAT(fecha_emision, 'yyyy-MM') AS month_key,
  ...
  async getCollectorPerformance(
    ...
    try {
      ...
      const query = `
        ...
        FROM Datos_Ingreso_Convenio
        WHERE estado_pago = 1 AND fecha_pago BETWEEN @Date_Start AND @Date_End;
        SELECT
            ...
            CASE WHEN @Total_Global = 0 THEN 0 ELSE CAST(SUM(valor_capital + valor_interes + valor_recargo) * 100.0 / @Total_Global AS DECIMAL(10,2)) END AS performance_pct
        FROM Datos_Ingreso_Convenio
        WHERE estado_pago = 1 AND fecha_pago BETWEEN @Date_Start AND @Date_End
    ...
    try {
      ...
      const query = `
        ...
        SELECT
            ...
            CASE WHEN @Total_Global = 0 THEN 0 ELSE CAST(SUM(valor_capital + valor_interes + valor_recargo) * 100.0 / @Total_Global AS DECIMAL(10,2)) END AS contribution_pct
        FROM Datos_Ingreso_Convenio
        WHERE estado_pago = 1 AND fecha_pago BETWEEN @Date_Start AND @Date_End
    ...
    try {
      ...
      const query = `
        ...
        SELECT
            ...
            CAST(COUNT(CASE WHEN di.estado_pago = 1 THEN 1 END) * 100.0 / NULLIF(COUNT(*), 0) AS DECIMAL(10,2)) AS collection_efficiency_pct,
